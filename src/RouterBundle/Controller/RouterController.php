...
class RouterController extends Controller {
	...
	public function customRouteAction($alias)
	{
		...
		$route = $em->getRepository('ContentBundle:MenuItem')->findOneBy(array('alias' => $slug));
	
		if (count($route) == 0)
		{
			$content = $em->getRepository('ContentBundle:Content')->findOneBy(array('alias' => $slug));
